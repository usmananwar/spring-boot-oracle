...
@Service
public class OverheadDataService {
	...
	public double getThrityThreeKvCount(boolean isZone1Included, boolean isZone2Included, boolean isZone3Included, String geometry) {
		...
		double result = 0;
		if (isZone1Included) {
			result = apiDataService.getFeaturesByZonesFromMapServer5(Constants.MedcZone1, whereClause, "SHAPE_Length", geometry);
		}

		if (isZone2Included) {
			result = result + apiDataService.getFeaturesByZonesFromMapServer5(Constants.MazonZone2, whereClause, "SHAPE_Length", geometry);
			result = result + apiDataService.getFeaturesByZonesFromMapServer5(Constants.TanweerZone2, whereClause, "SHAPE_Length", geometry);
		}

		if (isZone3Included) {
			result = result + apiDataService.getFeaturesByZonesFromMapServer5(Constants.MajanZone3, whereClause, "SHAPE.LEN", geometry);
		}
		return result;
	}
	...
	public double getOneVCount(boolean isZone1Included, boolean isZone2Included, boolean isZone3Included, String geometry) {
		...
		double result = 0;
		if (isZone1Included) {
			result = apiDataService.getFeaturesByZonesFromMapServer4(Constants.MedcZone1, whereClause, "SHAPE_Length", geometry);
		}

		if (isZone2Included) {
			result = result + apiDataService.getFeaturesByZonesFromMapServer4(Constants.MazonZone2, whereClause, "SHAPE_Length", geometry);
			result = result + apiDataService.getFeaturesByZonesFromMapServer4(Constants.TanweerZone2, whereClause, "SHAPE_Length", geometry);
		}

		if (isZone3Included) {
			result = result + apiDataService.getFeaturesByZonesFromMapServer4(Constants.MajanZone3, whereClause, "SHAPE.LEN", geometry);
		}
		return result;
	}
}
